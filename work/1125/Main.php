...
<?
	...
	$refresh = 2;
	$aIFTxRx = array();
	read_ifdata(&$refresh, &$aIFTxRx);  //read the filename and update parameter
	...
	hddinfo(); 
	echo "--------meminfo--------\n";
	meminfo(); //all/free/used%

	function fetchRxTx($devS, $devR) { //$devR used to query dev, $devS is $aIFTxRx key
		global $refresh, $aIFTxRx;

		$timestamp = time();
		$Min = date("i", $timestamp);
		if(intval($Min) % $refresh != 0) { //If the current time minute is not a multiple of refresh, the function is not executed
			return "";
		}
		...
		exec("/sbin/ip -s link show $devR", $ret); //query $devR dev,and save it to $ret

		$arx = listStringSplit(trim($ret[3])); 
		$rxflow = trim($arx[0]); //Rx :bytes
		$rxpack = trim($arx[1]); //packets

		$atx = listStringSplit(trim($ret[5]));
		$txflow = trim($atx[0]); //Tx :bytes
		$txpack = trim($atx[1]); //packets
	...
	function read_ifdata(&$refresh, &$aIFTxRx) {
		...
		if(!is_file($filename))	{//若無此檔案在後台則在後台創造
			$val = "refresh = " . $refresh;
	...
	function listStringSplit($sList) {//Separate strings with blanks and commas, and return a non-blank array
		$aList = preg_split("/[\s,]+/", $sList , -1, PREG_SPLIT_NO_EMPTY);
		return $aList;
	}

	function get_ethlink($dev) {//check the dev is connected
	...
	function get_ethflow($dev) {
		...
		$acoll = listStringSplit(trim($msg0[5])); //warrming: this is tx not collisions
